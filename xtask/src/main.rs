...
pub fn kind_for_path<P: AsRef<std::path::Path>>(path: P) -> Option<ArtifactRole> {
    ...
    if base_bytes.ends_with(b".dll") || base_bytes.ends_with(b".so") || base_bytes.ends_with(b".dylib"){
        return Some(ArtifactRole::SharedLibrary);
    }

    if base_bytes.ends_with(b".imp")  || base_bytes.ends_with(b".dll.imp") || base_bytes.ends_with(b".dll.lib") || base_bytes.ends_with(b".dll.a") {
    ...
    if contains_subslice(base_bytes, b".dll.") || contains_subslice(base_bytes, b".so.")  || contains_subslice(base_bytes, b".dylib.") {
...
fn build(b: BuildConfiguration) -> Result<BuildOutput, BuildError> {
    ...
    let artifacts = stream
        .filter_map(|message| 
            match message.unwrap() {
                ...
                Message::CompilerArtifact(artifact) if  artifact.package_id.repr.contains("rtori-core-ffi") => {
                    Some(artifact)
                },
                _ => None
            }
    ...
    let files = artifacts.filenames.into_iter().filter_map(|filename|
        {
    ...
    let output = command.wait().map_err(|e| BuildError::CouldNotGetExitStatus(e))?;    
    if output.success() {
...
fn main() {
    ...
    let cps = cps_deps::cps::Package {
        ...
        components: HashMap::from_iter(vec![
            (String::from("rtori-core-static"), cps_deps::cps::MaybeComponent::Component(  cps_deps::cps::Component::Archive(cps_deps::cps::ComponentFields {
