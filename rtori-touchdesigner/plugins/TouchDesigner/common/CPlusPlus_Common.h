...
public:
  ...
  // Returns a pointer to the instance of your CHOP_CPlusPlusBase,
  // TOP_CPlusPlusBase etc. subclass for this node that you've defined in your
  // project.
  virtual void *getNodeInstance(const PY_GetInfo &info,
                                void *reserved = nullptr) = 0;

  // If your python code is changing something in your node that should cause it
  // to re-cook you should call this at the end of your python code.
...
#define OP_PYTHON_STRUCT_HEADER                                                \
  int32_t OP_PY_STRUCT_HEADER[OP_STRUCT_HEADER_ENTRIES];
...
// For classes that we want to be able to reference count, this will be their
// base class. However management of the reference counting is done via the
// OP_SmartRef automatic reference counting.
class OP_RefCount {
public:
  virtual ~OP_RefCount() {}
...
public:
  ...
  OP_SmartRef(const OP_SmartRef<T> &t) : myTarget(nullptr) { operator=(t); }

  OP_SmartRef(OP_SmartRef<T> &&t) : myTarget(nullptr) {
    operator=(std::move(t));
  }

  ~OP_SmartRef() { release(); }

  void operator=(const OP_SmartRef<T> &t) {
  ...
  T *operator->() const { return myTarget; }

  operator bool() const { return myTarget != nullptr; }
...
public:
  ...
  // Minor version is used to denote upgrades to a plugin. It should be
  // increased when new features are added to a plugin that would cause loading
  // up a project with an older version of the plguin to behavior incorrectly.
  // For example if new parameters are added to the plugin. A project file will
  // expect the plugin installed on the system to be greater than or equal to
  // the plugin version the project was created with. Assuming the majorVersion
  // is the same.
  ...
  // If you provide either (or both) of these a custom Python class will be
  // created for your Custom OP that contains these getters/setters and/or
  // methods. These should be arrays of the given types, terminated by a {0}
  // entry (as it CPython standard for working with these types
  ...
  // similar to the way other Callback DATs are pre-filled in other nodes in
  // TouchDesigner. Note: This only works when the .dll is installed as a Custom
  // OP, not as a C++ OP.
...
// by another Custom OP who has a reference to it, via it's input or a
// parameter. The type of 'T' will be TOP_CPlusPlusBase, CHOP_CPlusPlusBase etc.
// depending on the OP family. Use the 'opType' field to verify that this node
// is the type you expect it to be, before casting 'instance' to your real class
// that implements the custom OP. Since the header of the Custom OP is needed to
// do the cast, this is only useful in cases where you are implementing multiple
// Custom OPs, and need a higher level of communication between them than
// parameters/inputs etc. The customOP field will be nullptr in the OP_TOPInput,
// OP_CHOPInput etc if the node is not a Custom OP.
//
// Does not work with plugins loaded directly into the CPlusPlus nodes.
template <class T> class OP_CustomOPInstance {
public:
  OP_CustomOPInstance() {
    instance = nullptr;
    opType = nullptr;
...
public:
  ...
  // 'op' which is the OP that the callback originated from (your Custom
  // Operator in this case). Use this function to create your 'arguments' tuple,
  // the first entry will already be filled with the PyObject for 'op'. You
  // should fill in the other entries you want, starting at index 1.
  ...
  // 'arguments' must be a PyTuple of arguments created with
  // createArgumentsTuple() 'keywords' must be nullptr or a PyDict of keyword
  // arguments that you create yourself. References to 'arguments' and
  // 'keywords' are not stolen, so Py_DECREF if you are done with them after
  // calling this function. This function will return the PyObject* returned by
  // the callback. This function will return nullptr on error, If non-nullptr is
  // returned, you are now the owner of it and must Py_DECREF it (or hold onto
  // it for other usages). If the node does not have a callback DAT created, or
  // no function matches the given functionName, then Py_None is returned.
  // Py_None must also have Py_DECREF called on it.
  virtual PyObject *callPythonCallback(const char *functionName,
                                       PyObject *arguments, PyObject *keywords,
                                       void *reserved1) = 0;

  // All CUDA operations must occur on the main thread, and between calls to
...
public:
  ...
  // The number of times this node has cooked. Incremented at the start of the
  // cook.
  uint32_t cookCount;
...
public:
  ...
  // Leave this as Invalid if you want to download the texture in it's GPU
  // native format. Only 2D textures can be converted to other formats.
  // 3D/Cube/2DArray all must have this set as Invalid.
  OP_PixelFormat pixelFormat;
};

class OP_TextureDesc {
public:
  OP_TextureDesc() { memset(reserved, 0, sizeof(reserved)); }
...
// When you are given one of these you become the owner. You need to call
// release() on it when you are done with it.
class OP_TOPDownloadResult : public OP_RefCount {
protected:
  virtual ~OP_TOPDownloadResult() {}

public:
  OP_TOPDownloadResult() { memset(reserved, 0, sizeof(reserved)); }

  // Stalls until the downloaded data is ready. If calling from the main thread,
  // try to avoid calling this for at least 1 frame to avoid a CPU stall (See
  // CPUMemoryTOP example). However, this call is thread safe so you can pass
  // this class off to another thread and have it stall right away and start
  // working on the data as soon as it's ready (such as outputting to an
  // external device).
...
public:
  OP_CUDAArrayInfo() { memset(reserved, 0, sizeof(reserved)); }

  // Description of the texture that cudaArray points to.
  OP_TextureDesc textureDesc;

  // When you first obtain a pointer to the TOP_CUDAArrayInfo, this will be
  // nullptr. It will get filled in with the correct memory address when you
  // call OP_Context::beginCUDAOperations()
...
public:
  ...
  // Call release() on it when you are done with it (or let the variable fall
  // out of scope and destruct itself).
  virtual OP_SmartRef<OP_TOPDownloadResult>
  downloadTexture(const OP_TOPInputDownloadOptions &opts,
                  void *reserved1) const = 0;

  // Can only be called from a C++ TOP/Custom TOP that is working in
  // TOP_ExecuteMode::CUDA. Will error/return nullptr in other cases. Should
  // only be called from within execute(), and the returned pointer will remain
  // valids until execute() returns. Returns a OP_CUDArrayInfo* that can be used
  // to get the cudaArray* pointer for the texture memory for this TOP.
  virtual const OP_CUDAArrayInfo *getCUDAArray(const OP_CUDAAcquireInfo &info,
                                               void *reserved2) const = 0;
...
class OP_String {
protected:
  OP_String() { memset(reserved, 0, sizeof(reserved)); }
...
public:
  ...
  const float *getChannelData(int32_t i) const { return channelData[i]; }

  // Retrieve the name of a specific channel.
  // 'i' ranges from 0 to numChannels-1
  // For example getChannelName(1) is the name of the 2nd channel

  const char *getChannelName(int32_t i) const { return nameData[i]; }
...
public:
  ...
  inline Vector &operator-=(const Vector &trans) {
    x -= trans.x;
    y -= trans.y;
    z -= trans.z;
    return *this;
...
class BoundingBox {
public:
  BoundingBox(float minx, float miny, float minz, float maxx, float maxy,
              float maxz)
  ...
  void enlargeBounds(const Position &pos) {
    if (pos.x < minX)
      minX = pos.x;
    if (pos.x > maxX)
      maxX = pos.x;
  ...
  void enlargeBounds(const BoundingBox &box) {
    if (box.minX < minX)
      minX = box.minX;
    if (box.maxX > maxX)
      maxX = box.maxX;
  ...
  bool getCenter(Position *pos) const {
    if (!pos)
      return false;
  ...
  // verifies if the input position (pos) is inside the current bounding box or
  // not:
...
public:
  ...
  virtual const SOP_CustomAttribData *
  getCustomAttribute(int32_t customAttribIndex) const = 0;

  // Returns the custom attribute data with its name
  virtual const SOP_CustomAttribData *
  getCustomAttribute(const char *customAttribName) const = 0;
  ...
  virtual bool sendRay(const Position &pos, const Vector &dir,
                       Position &hitPostion, float &hitLength,
                       Vector &hitNormal, float &hitU, float &hitV,
                       int &hitPrimitiveIndex) = 0;

  // Returns the SOP_PrimitiveInfo with primIndex
  const SOP_PrimitiveInfo &getPrimitive(int32_t primIndex) const {
  ...
  const int32_t *getAllPrimPointIndices() { return myPrimPointIndices; }
...
public:
  ...
  // this value is valid until the parameters are rebuilt or it is called with
...
public:
  ...
  // for menu types, int returns the menu selection index, string returns the
  // item
  ...
  // returns the requested value
  // this value is valid until the parameters are rebuilt or it is called with
  // the same parameter name. return value usable for life of parameter The
  // returned string will be in UTF-8 encoding.
  virtual const char *getParString(const char *name) const = 0;

  // this is similar to getParString, but will return an absolute path if it
  // exists, with slash direction consistent with O/S requirements. to get the
  // original parameter value, use getParString return value usable for life of
  // parameter The returned string will be in UTF-8 encoding.
  virtual const char *getParFilePath(const char *name) const = 0;

  // returns true on success
  // from_name and to_name must be Object parameters
  virtual bool getRelativeTransform(const char *from_name, const char *to_name,
                                    double matrix[4][4]) const = 0;
...
private:
  // Deprecated, only declared here so legacy code can work. Use the functions
  // in OP_TOPInput instead.
  virtual void *getTOPDataInCPUMemory(
      const OP_TOPInputOpenGL *top,
...
public:
  ...
  //
  // The returned object, if not null should have its reference count
  // decremented or else a memorky leak will occur.
...
public:
  ...
  // functions, and do not need to be retained by the calling function. Must
  // begin with capital letter, and contain no spaces
  const char *name;
  const char *label;
  const char *page;
...
public:
  ...
  // Any char* values passed are copied immediately by the append parameter
  // functions, and do not need to be retained by the calling function.
...
public:
  ...
  // The contents of the strings are copied during the call, you don't need to
  // keep copies around after the call returns.
...
public:
  ...
  // required, allowing you to fill the menu with custom entries based on other
...
static_assert(offsetof(OP_CustomOPInfo, minInputs) == 24,
              "Incorrect Alignment");
static_assert(offsetof(OP_CustomOPInfo, maxInputs) == 28,
              "Incorrect Alignment");
static_assert(offsetof(OP_CustomOPInfo, authorName) == 32,
              "Incorrect Alignment");
static_assert(offsetof(OP_CustomOPInfo, authorEmail) == 40,
              "Incorrect Alignment");
static_assert(offsetof(OP_CustomOPInfo, majorVersion) == 48,
              "Incorrect Alignment");
static_assert(offsetof(OP_CustomOPInfo, minorVersion) == 52,
              "Incorrect Alignment");
...
#ifdef _WIN32
static_assert(offsetof(OP_NodeInfo, mainWindowHandle) == 16,
              "Incorrect Alignment");
...
static_assert(offsetof(OP_TOPInput, totalCooks) == 156 + 20,
              "Incorrect Alignment");
...
static_assert(offsetof(OP_ObjectInput, worldTransform) == 16,
              "Incorrect Alignment");
static_assert(offsetof(OP_ObjectInput, localTransform) == 144,
              "Incorrect Alignment");
static_assert(offsetof(OP_ObjectInput, totalCooks) == 272,
              "Incorrect Alignment");
...
static_assert(offsetof(SOP_TextureInfo, numTextures) == 0,
              "Incorrect Alignment");
...
static_assert(offsetof(SOP_TextureInfo, numTextureLayers) == 16,
              "Incorrect Alignment");
...
static_assert(offsetof(SOP_CustomAttribData, numComponents) == 8,
              "Incorrect Alignment");
static_assert(offsetof(SOP_CustomAttribData, attribType) == 12,
              "Incorrect Alignment");
static_assert(offsetof(SOP_CustomAttribData, floatData) == 16,
              "Incorrect Alignment");
static_assert(offsetof(SOP_CustomAttribData, intData) == 24,
              "Incorrect Alignment");
static_assert(sizeof(SOP_CustomAttribData) == 32, "Incorrect Size");

static_assert(offsetof(SOP_PrimitiveInfo, numVertices) == 0,
              "Incorrect Alignment");
static_assert(offsetof(SOP_PrimitiveInfo, pointIndices) == 8,
              "Incorrect Alignment");
static_assert(offsetof(SOP_PrimitiveInfo, type) == 16, "Incorrect Alignment");
static_assert(offsetof(SOP_PrimitiveInfo, pointIndicesOffset) == 20,
              "Incorrect Alignment");
...
static_assert(offsetof(OP_NumericParameter, defaultValues) == 24,
              "Incorrect Alignment");
static_assert(offsetof(OP_NumericParameter, minValues) == 56,
              "Incorrect Alignment");
static_assert(offsetof(OP_NumericParameter, maxValues) == 88,
              "Incorrect Alignment");
static_assert(offsetof(OP_NumericParameter, clampMins) == 120,
              "Incorrect Alignment");
static_assert(offsetof(OP_NumericParameter, clampMaxes) == 124,
              "Incorrect Alignment");
static_assert(offsetof(OP_NumericParameter, minSliders) == 128,
              "Incorrect Alignment");
static_assert(offsetof(OP_NumericParameter, maxSliders) == 160,
              "Incorrect Alignment");
...
static_assert(offsetof(OP_TextureDesc, pixelFormat) == 16,
              "Incorrect Alignment");
...
static_assert(offsetof(OP_StringParameter, defaultValue) == 24,
              "Incorrect Alignment");
...
static_assert(offsetof(PY_Struct, context) ==
                  OP_STRUCT_HEADER_ENTRIES * sizeof(int32_t),
              "Incorrect Alignment");
}; // namespace TD
