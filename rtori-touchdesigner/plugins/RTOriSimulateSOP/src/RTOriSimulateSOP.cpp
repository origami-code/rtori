...
void GeneratorSOP::getGeneralInfo(SOP_GeneralInfo *ginfo,
...
void GeneratorSOP::execute(SOP_Output *output, const TD::OP_Inputs *inputs,
                           void *) {
...
void GeneratorSOP::executeVBO(SOP_VBOOutput *output,
                              const TD::OP_Inputs *inputs, void *) {
  // ShapeMenuItems shape = myParms.evalShape(inputs);
  // Color color = myParms.evalColor(inputs);
...
void GeneratorSOP::setupParameters(TD::OP_ParameterManager *manager, void *) {
  ...
  {
    OP_StringParameter datParameter;
    datParameter.name = "Foldsource";
    datParameter.label = "Fold Source";

    const OP_ParAppendResult res = manager->appendDAT(datParameter);
    assert(res == OP_ParAppendResult::Success);
  }

  {
    ...
    const OP_ParAppendResult res =
        manager->appendFloat(creasePercentageParameter, 1);
    assert(res == OP_ParAppendResult::Success);
  }
}