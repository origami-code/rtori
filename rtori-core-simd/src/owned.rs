    ...
    A: Allocator + 'allocator,
{
    pub fn with_size_in(size: &rtori_os_model::ModelSize, allocator: A) -> Self {
        ...
        // SAFETY:
        // We extend the lifetime, as know we will store them together all the time
        // And we won't invalidate the pointed to memory until the user (crate::Runner) is dropped
        let transmuted =
