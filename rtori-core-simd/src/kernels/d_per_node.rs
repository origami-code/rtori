        ...
        num_traits::Num + num_traits::NumAssign + SimdComplexField + SimdRealField,
{
    ...
    inputs.into_iter().map(move |per_node| {
        ...
        // /*2024-10-11*/ println!("mass: {:?} / positions: {:?} / force from crease {:?} / force from beam {:?} / force from face {:?}  / force (unscaled by dt): {:?} / dt: {:?}", *per_node.mass, position_offset, per_node.crease_force, per_node.beam_force, per_node.face_force, force, dt);
