    ...
    itertools::izip!(inputs.node_crease_indices, inputs.node_crease_node_number).map(
        |(crease_indices, node_number)| {
            ...
            let crease_face_indices = gather_scalar(
                [inputs.crease_face_indices[0], inputs.crease_face_indices[1]],
                *crease_indices,
            );
            ...
            let force_crease_reaction = {
                ...
                (algebrize(normal_a).scale(simba::simd::Simd(coef_a / crease_physics_a_height))
                    + algebrize(normal_b)
                        .scale(simba::simd::Simd(coef_b / crease_physics_b_height)))
            ...
            let force_other = {
                ...
                let face_indices = is_1.select(crease_face_indices[1], crease_face_indices[0]);
                let normals = gather_vec3f_1(&inputs.face_normals, face_indices);

                let moment_arm = is_1.select(crease_physics_b_coef, crease_physics_a_coef);
