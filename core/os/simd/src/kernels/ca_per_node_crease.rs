...
use crate::kernels::operations::gather::{gather_f32, gather_vec3f_1};
    ...
    simba::simd::Simd<core::simd::Simd<f32, L>>: nalgebra::SimdRealField,
{
 ...
 // Now it's time to load the geometry
            ...
            let force_crease_reaction = {
                ...
                let side = #[inline]
                |normal, coef, height| {
                    to_nalgebra3(normal).scale(simba::simd::Simd(coef / height))
                };
            ...
            let force_complementary = {
                ...
                to_nalgebra3(normals).scale(simba::simd::Simd(angular_force / moment_arm))
            };
