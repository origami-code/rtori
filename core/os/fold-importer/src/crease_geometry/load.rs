...
fn load_node_position<'output, O, I>(output: &mut O, input: &I)
where
...
fn load_faces<'output, O, I>(output: &mut O, input: &I) -> Result<(), ImportError>
where
    O: rtori_os_model::LoaderDyn<'output> + 'output,
    I: crate::ImportInput,
{
...
fn load_creases<'output, O, I>(
    output: &mut O,
    input: &I,
    creases: &[crate::creases::Crease],
    ...
    I: crate::ImportInput,
{
    for (crease_index, crease) in creases.iter().enumerate() {
        ...
        let vertex_indices = input
            ...
            .ok_or(ImportError::InvalidCrease {
        ...
        let vertices = vertex_indices.try_map({
            let mut edge_member = 0;
            move |vertex_index| {
                let result = input.vertices_coords().get(vertex_index as usize).ok_or(
                    ImportError::EdgesVerticesPointingToInvalidVertex {
                        edge_index: crease.edge_index,
                        edge_member,
                        pointing_to: vertex_index,
                        vertex_count: input.vertices_coords().count() as u32,
                    },
                );
                edge_member += 1;
...
fn load_node_geometry<'output, O, I, SA>(
    output: &mut O,
    input: &I,
    creases: &[crate::creases::Crease],
    node_inv_creases: &[super::CreaseNodePair],
    node_creases: &[super::CreaseNodePair],
    config: crate::ImportConfig,
    allocator: SA,
) -> Result<(), ImportError>
where
    ...
    SA: core::alloc::Allocator + Clone,
{
    ...
    where
        S: AsRef<[crate::crease_geometry::CreaseNodePair]>,
        A: Allocator + Clone,
    {
        let mut mapping = alloc::vec::Vec::with_capacity_in(vertices_count, allocator.clone());
        // TODO: use a fixed capacity here for the switch to bump allocators
    ...
    .enumerate()
    {
        ...
        // Load node-crease
        {
            fn process_node_crease<'input, 'loader, L, EV>(
                ...
                crease_index: u32,
                crease: &crate::creases::Crease,
                edges_vertices: &EV,
            ) where
                L: rtori_os_model::LoaderDyn<'loader>,
                EV: crate::Proxy<'input, Output = [u32; 2]>,
            {
        ...
        // Load node-beams
        {
            let edge_vertex_indices: <I as crate::ImportInput>::EdgeVertices<'_> =
                input.edges_vertices();
            vertices_edges
                .iter()
                .map(|edge_index| {
                    (
                ...
                .for_each(
                    |(node_beam_local_index, (edge_index, edge_vertex_indices))| {
                        ...
                        let spec = rtori_os_model::NodeBeamSpec {
                            node_index,
                            k,
                            d,
                            length,
    ...
    A: core::alloc::Allocator,
{
    ...
    where
        ...
        SA: core::alloc::Allocator + Clone,
    {
        let crease_geometry = &self.crease_geometry;
        let input = self.input;
        let creases = &crease_geometry.creases;
        let node_inv_creases = &crease_geometry.node_creases_adjacent;
        let node_creases = &crease_geometry.node_creases_complement;

        load_node_position(output, input);
        load_faces(output, input)?;
        load_creases(output, input, creases, config)?;
        load_node_geometry(
            output,
            input,
            creases,
