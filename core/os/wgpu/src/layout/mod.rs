...
impl PipelineSetLayout {
    pub fn new(device: &wgpu::Device) -> Self {
        let bgl_positions_ro =
            device.create_bind_group_layout(&bgld::positions_ro::BIND_GROUP_POSITIONS_RO);
        let bgl_per_crease_common = device
            .create_bind_group_layout(&bgld::per_crease_common::BIND_GROUP_PER_CREASE_GEOMETRY);

        let pass_per_face = PassPerFace::new(device, &bgl_positions_ro);
        let pass_per_crease_fold_angle =
            PassPerCreaseFoldAngle::new(device, &bgl_positions_ro, &bgl_per_crease_common);
        let pass_per_crease_physics =
            PassPerCreasePhysics::new(device, &bgl_positions_ro, &bgl_per_crease_common);
        let pass_per_node_crease =
            PassPerNodeCrease::new(device, &bgl_positions_ro, &bgl_per_crease_common);
