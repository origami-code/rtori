...
use std::sync::Arc;
type RawAlloc = unsafe extern fn(ctx: *const c_void, size: usize, alignment: usize) -> *mut c_void;
type RawDealloc = unsafe extern fn(ctx: *const c_void, ptr: *mut c_void, size: usize, alignment: usize);
...
unsafe impl<'alloc> Allocator for &'alloc CustomAllocator {
    ...
    unsafe fn deallocate(
        ...
        unsafe {(self.dealloc)(self.ctx, ptr.as_ptr().cast::<c_void>(), layout.size(), layout.align())};
    }
}
...
#[no_mangle]
pub unsafe extern "C" fn rtori_init(
    parameters: Option<&CustomAllocator>
) -> *const Context<'_> {
    let allocator = parameters
        .map(|custom| ContextAllocator::Custom(custom)).unwrap_or(ContextAllocator::Global);

    let context = Context {
        allocator
    };

    let boxed = Arc::new_in(
        context,
        allocator
    );
...
#[no_mangle]
pub unsafe extern "C" fn rtori_clone(
    ctx: *const Context<'_>
) -> *const Context<'_> {
...
#[no_mangle]
pub unsafe extern "C" fn rtori_deinit(
    ctx: *const Context
) {
