...
pub trait Proxy {
    type Output;
    fn count(&self) -> usize;
    fn get<'a>(&'a self, idx: usize) -> Option<Self::Output>;
    fn iter<'a>(&'a self) -> impl Iterator<Item=Self::Output>;
}

impl<T> Proxy for &[T] where T: Copy {
    type Output = T;
...
impl<'a> FoldInput for &'a fold::FrameCore {
    ...
    fn edges_assignment(&self) -> Self::EA {
       FoldAssignmentParser(self.edges.assignments.as_ref().map(|v| v.as_slice()).unwrap_or(&[]))
    }
...
fn import<I: FoldInput>(input: I) -> Processed {

}

fn extract_creases(input: &fold::FrameCore) -> Processed {
    let iterator = fold::macros::iter!(
        input.edges,
        vertices,
        assignment,
        faces,
