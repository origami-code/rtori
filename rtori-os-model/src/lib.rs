...
pub trait Destination {
    ...
    fn set_face_indices(&mut self, face_idx: FaceIndex, parameters: Vector3U);
    fn set_face_nominal_angles(&mut self, face_idx: FaceIndex, parameters: Vector3F);
...
pub trait MappedDestination<'container> {
    type NodePositionMap<'a>: DerefMut<Target = [Vector3F]> + 'a where Self: 'a, 'a: 'container;
    fn map_node_position<'call>(&'call mut self) -> Self::NodePositionMap<'call> where 'call: 'container;

    type NodeExternalForcesMap<'a>: DerefMut<Target = [Vector3F]> + 'a where Self: 'a, 'a: 'container;
    fn map_node_external_forces<'call>(&'call mut self) -> Self::NodeExternalForcesMap<'call>  where 'call: 'container;

    type NodeConfigMap<'a>: DerefMut<Target = [NodeConfig]> + 'a where Self: 'a, 'a: 'container;
    fn map_node_config<'call>(&'call mut self) -> Self::NodeConfigMap<'call>  where 'call: 'container;

    type NodeGeometryMap<'a>: DerefMut<Target = [NodeGeometry]> + 'a where Self: 'a, 'a: 'container;
    fn map_node_geometry<'call>(&'call mut self) -> Self::NodeGeometryMap<'call>  where 'call: 'container;

    type CreaseGeometryMap<'a>: DerefMut<Target = [CreaseGeometry]> + 'a where Self: 'a, 'a: 'container;
    fn map_crease_geometry<'call>(&'call mut self) -> Self::CreaseGeometryMap<'call>  where 'call: 'container;

    type CreaseParametersMap<'a>: DerefMut<Target = [CreaseParameters]> + 'a where Self: 'a, 'a: 'container;
    fn map_crease_parameters<'call>(&'call mut self) -> Self::CreaseParametersMap<'call>  where 'call: 'container;

    type FaceIndicesMap<'a>: DerefMut<Target = [Vector3U]> + 'a where Self: 'a, 'a: 'container;
    fn map_face_indices<'call>(&'call mut self) -> Self::FaceIndicesMap<'call>  where 'call: 'container;

    type FaceNominalAnglesMap<'a>: DerefMut<Target = [Vector3F]> + 'a where Self: 'a, 'a: 'container;
    fn map_face_nominal_angles<'call>(&'call mut self) -> Self::FaceNominalAnglesMap<'call>  where 'call: 'container;

    type NodeCreaseMap<'a>: DerefMut<Target = [NodeCreaseSpec]> + 'a where Self: 'a, 'a: 'container;
    fn map_node_crease<'call>(&'call mut self) -> Self::NodeCreaseMap<'call>  where 'call: 'container;

    type NodeBeamMap<'a>: DerefMut<Target = [NodeBeamSpec]> + 'a where Self: 'a, 'a: 'container;
    fn map_node_beam<'call>(&'call mut self) -> Self::NodeBeamMap<'call>  where 'call: 'container;

    type NodeFaceMap<'a>: DerefMut<Target = [NodeFaceSpec]> + 'a where Self: 'a, 'a: 'container;
    fn map_node_face<'call>(&'call mut self) -> Self::NodeFaceMap<'call>  where 'call: 'container;
}
