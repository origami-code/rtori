...
pub extern "C" fn Context_custom(custom: std::ptr::NonNull<crate::CustomAllocator>) -> *const ffi::Context<'static> {
...
mod ffi {
    ...
    impl Context<'static> {
        ...
        pub const fn global() -> Box<Self, crate::ContextAllocator<'static>> {
            todo!()
        }
    }
    ...
    pub struct FoldFileParseError {
        status: FoldFileParseErrorKind,
        error: DiplomatOption<JSONParseError>
    }

    impl<'ctx> FoldFile<'ctx> {
        pub fn parse_bytes(ctx: &Context<'ctx>, bytes: &[DiplomatByte]) -> Result<Box<FoldFile<'ctx>, crate::ContextAllocator<'ctx>>, FoldFileParseError> {
            if bytes.len() == 0 {
            ...
            let parsed = serde_json::from_slice::<fold::File>(bytes)
                .map_err(|inner| FoldFileParseError{
        ...
        pub fn parse_str(ctx: &Context<'ctx>, str: &str) -> Result<Box<FoldFile<'ctx>, crate::ContextAllocator<'ctx>>, FoldFileParseError> {
            if str.len() == 0 {
                return Err(FoldFileParseError {
                    status: FoldFileParseErrorKind::Empty,
                    error: DiplomatOption::from(None)
            ...
            let parsed = serde_json::from_str::<fold::File>(str)
                .map_err(|inner| FoldFileParseError{
                    status: FoldFileParseErrorKind::Error,
                    error: DiplomatOption::from(Some(JSONParseError::from(inner)))
                })?;
        ...
        pub fn query_metadata_string(&self, field: FoldMetadataQuery, output: &mut DiplomatWrite) -> Result<(), ()> {
            if !matches!(field, FoldMetadataQuery::Creator | FoldMetadataQuery::Author) {
                return Err(());
            }

            let result_str = self.inner.file_metadata.as_ref().and_then(|metadata| match field {
