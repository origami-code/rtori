...
impl PassPerCreasePhysics {
    pub fn new(
        ...
        let bgl_specific = device.create_bind_group_layout(&super::bgld::per_crease_physics::BIND_GROUP_PER_CREASE_PHYSICS);

        let shader_module =  device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("rtori-cs-per-crease-physics"),
            source: wgpu::ShaderSource::Wgsl(Cow::Borrowed(include_str!("../../../shaders/1-PerCrease-Physics.wgsl")))
        });
