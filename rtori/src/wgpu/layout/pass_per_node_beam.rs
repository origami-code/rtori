...
impl PassPerNodeBeam {
    pub fn new(
        ...
        let bgl_specific = device.create_bind_group_layout(&super::bgld::per_node_beam::BIND_GROUP_PER_NODE_BEAM);

        let shader_module =  device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("rtori-cs-per-node-beam"),
            source: wgpu::ShaderSource::Wgsl(Cow::Borrowed(include_str!("../../../shaders/2b0-PerNodeBeam.wgsl")))
        });
