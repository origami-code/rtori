...
@group(1)
@binding(0)
var<storage, read_write> node_face_constraint_output: array<vec4<f32>>; // x, y, z, error
...
fn per_node_face(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    node_face_constraint_output[node_face_index] = vec4<f32>(constraint_results.force, constraint_results.error);
}