...
@group(0)
@binding(0)
var<storage, read_write> node_position_offset_output: array<vec3<f32>>;
...
@group(0)
@binding(3)
var<storage, read> node_position_offset_input: array<vec3<f32>>;
...
@group(0)
@binding(6)
var<storage, read> node_face_constraint_output: array<vec4<f32>>; // x, y, z, error_accor
...
fn compute_force(node_index: u32) -> ConstraintResult {
    ...
    for (var f: u32; f < geometry.face_count; f++) {
        ...
        var constraint_result = node_face_constraint_output[node_beam_index];
        force_acc += constraint_result.xyz;
        error_acc += constraint_result.w;
    }
...
@compute
@workgroup_size(1)
fn per_node(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (is_fixed) {
        ...
        node_position_offset_output[node_index] = node_position_offset_input[node_index]
        node_error[node_index] = 0.0;
    } else {
        node_position_offset_output[node_index] = node_position_offset_input[node_index] + new_velocity * dt;
